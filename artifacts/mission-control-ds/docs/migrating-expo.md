# Migrating Expo UI to Mission Control Design System

Read `artifacts/mission-control-ds/docs/AGENTS.md` and
`artifacts/mission-control-ds/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/mission-control-ds/lib/native-theme` and
  `@workspace/mission-control-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/mission-control-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/mission-control-ds` and retains only
